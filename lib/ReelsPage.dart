...
import 'package:shared_preferences/shared_preferences.dart';
import 'package:firebase_analytics/firebase_analytics.dart';
import 'package:google_mobile_ads/google_mobile_ads.dart'; // Added for ads integration
...
class _ReelsPageState extends State<ReelsPage> {
  ...
  // Ads
  InterstitialAd? _interstitialAd; // Interstitial Ad object for displaying between reels
  bool _isAdLoaded = false; // Track ad loading status
  RewardedAd? _rewardedAd; // Rewarded Ad for rewarding users on certain actions
  ...
  void initState() {
    super.initState();
    _initializeUser();
    _loadInterstitialAd(); // Load ads when initializing
    _loadRewardedAd(); // Load rewarded ads for bonus features
  }
    ...
    RewardedAd.load(
      ...
      rewardedAdLoadCallback: RewardedAdLoadCallback(
        onAdLoaded: (ad) {
          ...
          _setRewardedAdCallbacks(); // Set callbacks for the loaded rewarded ad
        },
        onAdFailedToLoad: (error) {
    ...
    if (_rewardedAd != null) {
      _rewardedAd!.show(
        onUserEarnedReward: (AdWithoutView ad, RewardItem reward) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("You've earned a reward for watching the ad!")),
          );
          // Additional logic to reward user
        },
      );
      setState(() {
        _rewardedAd = null;
  ...
  // Enhanced logout with analytics event
  Future<void> _logoutEnhanced() async {
    try {
      await _auth.signOut();
      Navigator.pushReplacementNamed(context, '/login');
  ...
  Widget _buildBottomNavigationBar() {
    return BottomNavigationBar(
      ...
      selectedItemColor: Color(0xFFE17055),
      unselectedItemColor: Color(0xFF8D6E63),
      currentIndex: 3, // Current index set to Reels
      items: [
      ...
      onTap: (index) {
        if (_routes[index] == '/reels') return; // Avoid navigating to the same page
    ...
    return Scaffold(
      ...
      body: isLoading
        ...
        itemBuilder: (context, index) {
          ...
          // Show Interstitial Ad after every 3rd reel
                  ...
                  children: [
                    ...
                    IconButton(
                      icon: Icon(Icons.comment,
                          color: Color(0xFF8D6E63)),
                      onPressed: () {
                        _showCommentsModal(reel["uid"],
                            reel["imageId"], reel["comments"]);
                      },
                    ),
                    if (reel["uid"] != currentUser!.uid)
                      IconButton(
  ...
  Future<void> _addComment(String uid, String imageId, String commentText) async {
    ...
    try {
      ...
      setState(() {
        for (var reel in reelsData) {
          if (reel["uid"] == uid && reel["imageId"] == imageId) {
            // Insert the new comment at the beginning for the latest first
            reel["comments"].insert(0, {
              "user": currentUser!.email?.split('@')[0] ?? 'Anonymous',
              "text": commentText,
            });
